...
async function handleDuplicateDetection() {
  ...
  try {
    ...
    uiManager.showNotification(
      'Searching for potential duplicate projects...',
      'info'
    );
    ...
    if (result) {
      ...
      console.log(
        'Result has duplicateGroups property:',
        result.hasOwnProperty('duplicateGroups')
      );
      if (result.duplicateGroups) {
        console.log(
          'Duplicate groups is array:',
    ...
    logger.debug(
      'duplicateDetector',
      'Checking duplicate detection result',
      result
    );
    ...
    if (!result) {
      ...
      console.error(
        'CRITICAL ERROR: Result indicates failure',
      ...
      console.error(
        'CRITICAL ERROR: duplicateGroups is not an array',
        typeof result.duplicateGroups
      );
    ...
    if (
      ...
      console.log(
        `Found ${duplicateGroups.length} potential duplicate groups to review`
      );
      logger.debug(
        'duplicateDetector',
        `Found ${duplicateGroups.length} potential duplicate groups to review`
      ...
      duplicateGroups.forEach((group, groupIndex) => {
        logger.debug(
          'duplicateDetector',
          `Group ${groupIndex + 1} has ${group.length} projects:`
        );
        group.forEach((project, projectIndex) => {
          logger.debug(
            'duplicateDetector',
            `  Project ${projectIndex + 1}: ${project.title || 'Untitled'} - ${project.path || 'No path'}`
          );
        });
      });
      ...
      uiManager.showNotification(
        `Found ${duplicateGroups.length} potential duplicate groups ready for review`,
        'success'
      );

      // 4. Make sure duplicate review container exists
      const duplicateReviewContainer = document.getElementById(
      ...
      if (result && result.message) {
        ...
        uiManager.showNotification(
          'No potential duplicate projects found',
          'info'
        );
      }
    ...
    uiManager.showNotification(
      'Error detecting duplicates: ' + error.message,
      'error'
    );
  }
}
...
function showDuplicateReviewContainer() {
  ...
  try {
    ...
    if (!duplicateReviewContainer) {
      console.error(
        'CRITICAL: Duplicate review container not found in the DOM'
      );
      ...
      if (duplicatesTab) {
        ...
        if (debugPanel && debugPanel.parentNode) {
          ...
          debugPanel.parentNode.insertBefore(
            newContainer,
            debugPanel.nextSibling
          );
        } else {
    ...
    console.log(
      'Made duplicate review container visible with !important style'
    );
    ...
    if (duplicateList) {
      ...
      if (duplicateReviewContainer) {
        ...
        const counter = duplicateReviewContainer.querySelector(
          '.duplicate-group-counter'
        );
...
function createDuplicateReviewContainer() {
  ...
  if (duplicatesTab) {
    ...
    const insertAfter =
      duplicatesTab.querySelector('.review-actions') ||
    ...
    const activeTab =
      document.getElementById('active-tab') ||
...
function displayDuplicateGroup() {
  ...
  console.log(
    'Displaying duplicate group with',
    currentDuplicateGroup.length,
    'projects'
  );
  logger.debug(
    'duplicateDetector',
    `Displaying duplicate group ${currentDuplicateIndex + 1}/${duplicateGroups.length} with ${currentDuplicateGroup.length} projects`
  );
  ...
  currentDuplicateGroup.forEach((project, idx) => {
    logger.debug(
      'duplicateDetector',
      `  Project ${idx + 1}: ${project.title || 'Untitled'} - ${project.path || 'No path'}`
    );
  });
  ...
  console.log(
    `Creating ${currentDuplicateGroup.length} project containers for duplicate group`
  );
  currentDuplicateGroup.forEach((project, index) => {
    console.log(
      `Creating container for project ${index}: ${project.title || 'Untitled'}`
    );
...
async function mergeSelectedProjects() {
  if (selectedProjects.size < 2) {
    uiManager.showNotification(
      'Please select at least 2 projects to merge',
      'warning'
    );
    return;
  }
  ...
  try {
    ...
    const projectsToMerge = currentDuplicateGroup.filter(project =>
    ...
    if (
      confirm(
        `Are you sure you want to merge ${projectsToMerge.length} projects? This action cannot be undone.`
      )
    ) {
      // Call the backend to merge projects
      const result = await projectData.mergeProjects(
        projectsToMerge.map(p => p.path)
      );
    ...
    uiManager.showNotification(
      'Error merging projects: ' + error.message,
      'error'
    );
...
async function startDuplicateDetection() {
  console.log(
    'startDuplicateDetection called, forwarding to handleDuplicateDetection'
  );
