...
function generateReport() {
  ...
  if (projects.active) {
    wellFormulatedActive = projects.active.filter(
      p => p.isWellFormulated
    ).length;
  }

  if (projects.waiting) {
    wellFormulatedWaiting = projects.waiting.filter(
      p => p.isWellFormulated
    ).length;
  }

  if (projects.someday) {
    wellFormulatedSomeday = projects.someday.filter(
      p => p.isWellFormulated
    ).length;
  }
  ...
  const activePercentage =
    activeCount > 0
      ? Math.round((wellFormulatedActive / activeCount) * 100)
      : 0;
  const waitingPercentage =
    waitingCount > 0
      ? Math.round((wellFormulatedWaiting / waitingCount) * 100)
      : 0;
  const somedayPercentage =
    somedayCount > 0
      ? Math.round((wellFormulatedSomeday / somedayCount) * 100)
      : 0;
  ...
  if (recentlyArchivedProjects.length > 0) {
    ...
    recentlyCompleted.innerHTML +=
      '<p>No projects completed in the last 30 days</p>';
  }
