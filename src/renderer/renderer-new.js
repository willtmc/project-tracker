  ...
  if (gridViewBtn && listViewBtn) {
    ...
    if (currentView === 'list') {
      ...
      projectGrids.forEach(grid => {
        ...
        console.log('Added list-view class to grid:', grid.id || 'unnamed grid');
      });
    } else {
      ...
      projectGrids.forEach(grid => {
        ...
        console.log('Removed list-view class from grid:', grid.id || 'unnamed grid');
      });
    }
    ...
    gridViewBtn.addEventListener('click', () => {
      ...
      console.log('Updating all project grids to grid view:', allProjectGrids.length);
      
      allProjectGrids.forEach(grid => {
        console.log('Removing list-view class from grid:', grid.id || 'unnamed grid');
        grid.classList.remove('list-view');
      });
    });
    ...
    listViewBtn.addEventListener('click', () => {
      ...
      // Get all project grids again to ensure we have the latest
      const allProjectGrids = document.querySelectorAll('.projects-grid');
      console.log('Updating all project grids to list view:', allProjectGrids.length);
      
      allProjectGrids.forEach(grid => {
        console.log('Adding list-view class to grid:', grid.id || 'unnamed grid');
        grid.classList.add('list-view');
      });
    });
  } else {
  ...
  if (searchTerm) {
    ...
    filteredProjects = projects.filter(project => {
      return (
        ...
        (project.waitingInput && project.waitingInput.toLowerCase().includes(term))
      );
    });
  }
  ...
  if (sortOption) {
    filteredProjects.sort((a, b) => {
      switch (sortOption) {
        ...
        case 'progress-asc':
          const progressAsc = a.totalTasks > 0 ? (a.completedTasks / a.totalTasks) : 0;
          const progressBsc = b.totalTasks > 0 ? (b.completedTasks / b.totalTasks) : 0;
...
function renderProjectsByCategory(projects) {
  ...
  const sortOption = document.getElementById('filter-sort')?.value || 'modified-desc';
  ...
  if (projects.active && activeContainer) {
    const filteredProjects = filterAndSortProjects(projects.active, searchTerm, statusFilter, sortOption);
  ...
  if (projects.waiting && waitingContainer) {
    const filteredProjects = filterAndSortProjects(projects.waiting, searchTerm, statusFilter, sortOption);
  ...
  if (projects.someday && somedayContainer) {
    const filteredProjects = filterAndSortProjects(projects.someday, searchTerm, statusFilter, sortOption);
  ...
  if (projects.archive && archiveContainer) {
    const filteredProjects = filterAndSortProjects(projects.archive, searchTerm, statusFilter, sortOption);
...
function createProjectElement(project, category) {
  ...
  statusIndicator.textContent = category.charAt(0).toUpperCase() + category.slice(1);
  ...
  if (project.totalTasks > 0) {
    ...
    const progressPercent = Math.round((project.completedTasks / project.totalTasks) * 100);
  ...
  if (project.endState) {
    ...
    const truncatedEndState = project.endState.length > 100 
...
function registerIPCHandlers() {
  ...
  ipcRenderer.on('database-restored', (event, data) => {
    ...
    uiManager.showNotification(data.message || 'Database has been restored', 'success');
