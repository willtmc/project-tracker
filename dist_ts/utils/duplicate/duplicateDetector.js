...
class DuplicateDetector {
                        ...
                        duplicateGroups = parsedGroups.map(group => {
                            return group
                                ...
                                if (index >= 0 && index < activeProjects.length) {
                                    ...
                                    console.error(`Invalid index ${index} in duplicate group, max index is ${activeProjects.length - 1}`);
                                    return null;
                                }
                            })
        ...
        try {
            ...
            for (let i = 0; i < projects.length; i++) {
                ...
                for (let j = i + 1; j < projects.length; j++) {
                    ...
                    const isSimilar = this.similarityChecker.checkBasicTextSimilarity(currentProject, otherProject);
                    if (isSimilar) {
            ...
            console.log(`Found ${duplicateGroups.length} potential duplicate groups using basic similarity`);
            return duplicateGroups;
        }
        catch (error) {
    ...
    async checkProjectSimilarity(project1, project2) {
        try {
            ...
            return await this.aiSimilarityChecker.checkProjectSimilarityWithAI(project1, project2);
        }
        catch (error) {
            console.error('AI similarity check failed, falling back to basic similarity check:', error);
            // Fallback to basic text similarity if AI fails
            return this.similarityChecker.checkBasicTextSimilarity(project1, project2);
        }
    }
    /**
        ...
        try {
            ...
            const response = await this.openAIService.generateChatCompletion({
                ...
                messages: [
                    {
                        role: 'system',
                        content: 'You are a specialized assistant that merges two similar projects into a single, comprehensive project. ' +
