...
class ProjectManager {
    ...
    validateProjectStructure(content, filename = '') {
        ...
        const filenameReflectsEndState = filename &&
            endStateContent &&
            (filename
                .toLowerCase()
                .includes(endStateContent.toLowerCase().substring(0, 10)) ||
                endStateContent.toLowerCase().includes(filename
                    .replace(/\.txt$/, '')
                    .toLowerCase()
                    .substring(0, 10)));
        // Basic validation - must have all required sections
        const isWellFormulated = hasTitle &&
            hasEndState &&
            hasTasks &&
            hasAdditionalInfo &&
        ...
        const needsImprovement = hasTitle &&
            hasEndState &&
    ...
    async updateProjectStatus(projectPath, isActive, isWaiting, waitingInput, targetStatus) {
        try {
            ...
            if (targetStatus &&
                ['active', 'waiting', 'someday', 'archive'].includes(targetStatus)) {
            ...
            if (newStatus !== previousStatus) {
                ...
                if (newStatus === 'waiting' && waitingInput) {
                    ...
                    if (/##\s+Waiting\s+on\s+Inputs/i.test(content)) {
                        ...
                        updatedContent = content.replace(/(##\s+Waiting\s+on\s+Inputs\s*\n)[\s\S]*?(?=\n##|$)/i, `$1- This project is waiting on: ${waitingInput}\n`);
                    }
                    else {
...
...`;
            ...
            const completion = await openai.chat.completions.create({
                ...
                messages: [
                    {
                        role: 'system',
                        content: 'You are a helpful assistant that reformulates project descriptions to ensure they are well-structured and complete.',
                    },
        ...
        try {
            ...
            for (const group of duplicateGroups) {
                for (const project of group) {
                    await Project.update({ hasPotentialDuplicates: true }, { where: { filename: project.filename } });
                }
            }
