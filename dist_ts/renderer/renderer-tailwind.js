        ...
        result = result.filter(project => {
            ...
            (project.title && project.title.toLowerCase().includes(searchTerm)) ||
                ...
                (project.tasks &&
                    ...
                    project.tasks.some(task => task.title && task.title.toLowerCase().includes(searchTerm))));
        });
    }
    // Apply status filter
...
async function startDuplicateDetection() {
    try {
        ...
        try {
            ...
            showNotification('Error detecting duplicates: ' + apiError.message, 'error');
            if (startDuplicateDetectionBtn) {
                hideLoading(startDuplicateDetectionBtn);
            }
        }
...
function displayDuplicateGroup(groupIndex) {
    try {
        ...
        const actionButtons = document.querySelector('#duplicate-review-container .flex.gap-3');
      ...
      `;
            ...
            currentGroup.forEach((project, index) => {
                ...
                if (project.path)
                    checkbox.dataset.projectPath = project.path;
...
window.formulateProjects = formulateProjects;
window.viewReport = viewReport;
window.skipDuplicate = skipDuplicate;
window.endDuplicateReview = endDuplicateReview;
...
async function startProjectReview() {
    try {
        ...
        // Update UI
        if (reviewTotal)
            reviewTotal.textContent = activeProjectsForReview.length;
        if (reviewCount)
        ...
        if (activeProjectsForReview.length > 0) {
            ...
            if (reviewProjectContent)
                reviewProjectContent.textContent = '';
...
function showProjectForReview(index) {
    try {
        ...
        if (reviewCount)
            reviewCount.textContent = index + 1;
    }
    catch (error) {
...
async function moveProjectToSomeday(project) {
    try {
        ...
        showNotification('Error moving project to Someday: ' +
  ...
  `;
    ...
    saveBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            showNotification('Error moving project to Waiting: ' +
                (error.message || 'Unknown error'), 'error');
        }
    });
...
//# sourceMappingURL=renderer-tailwind.js.map