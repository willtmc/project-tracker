...
async function startDuplicateDetection() {
    ...
    try {
        ...
        if (result.success && result.duplicateGroups && result.duplicateGroups.length > 0) {
            ...
            showNotification('No potential duplicate projects found', 'info');
        }
    }
    catch (error) {
        console.error('Error in duplicate detection:', error);
        showNotification('Error searching for duplicates', 'error');
    }
}
/**
...
async function mergeDuplicateProjects() {
    ...
    try {
        // Show loading notification
        showNotification('Merging duplicate projects...', 'info');
        // Get project paths
        ...
        if (result.success) {
            ...
            showNotification(`Error merging projects: ${result.message}`, 'error');
        }
    }
    catch (error) {
        console.error('Error merging duplicate projects:', error);
        showNotification('Error merging projects', 'error');
    }
}
...
function startDuplicateReview() {
    ...
    if (!duplicateGroups || duplicateGroups.length === 0) {
        showNotification('No duplicate groups to review', 'info');
        return;
    }
    // Set review mode
    isReviewMode = true;
    isReviewingDuplicates = true;
    // Set current duplicate group
    ...
    const reviewInstructions = document.querySelector('.review-instructions');
    ...
    `;
        ...
        document.getElementById('cancel-duplicate-review-btn').addEventListener('click', endDuplicateReview);
    }
...
function displayDuplicateGroup() {
    ...
    console.log('Displaying duplicate group');
    ...
    `;
        ...
        projectContent.textContent = project.content.substring(0, 300) + '...';
...
function moveToNextDuplicateGroup() {
    console.log('Moving to next duplicate group');
    if (!duplicateGroups || duplicateGroups.length === 0) {
        console.error('No duplicate groups available');
        showNotification('No more duplicate groups to review', 'info');
        endDuplicateReview();
        return;
    }
    // Increment index
    const currentIndex = duplicateGroups.indexOf(currentDuplicateGroup);
    if (currentIndex < 0) {
        console.error('Current duplicate group not found in list');
        return;
    }
    ...
    if (currentIndex >= duplicateGroups.length - 1) {
        ...
        showNotification('All duplicate groups reviewed', 'success');
        endDuplicateReview();
        return;
    }
...
function endDuplicateReview() {
    ...
    loadProjects();
    // Show notification
    showNotification('Duplicate review ended', 'info');
}
// Add event listeners
